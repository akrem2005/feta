...
class Mesh {
    constructor(client) {
        ...
        this.updateQueue = (0, async_1.priorityQueue)(async (time, done) => {
            ...
            const totalMaxBitrate = appliedTier[1].totalOutgoingBitrate ?? 1800000;
            const perVideoBitrate = totalMaxBitrate / Math.min((this.sfu.enabled ? 1 : videoPeersCount) * sharedVideoCount, 1);
            const sessionMaxBitrate = Math.min(appliedTier[1].bitrate ?? perVideoBitrate, perVideoBitrate);
            ...
            for (const call of calls) {
                const room = Selectors.getRoomByAddress(state, call.roomAddress);
                ...
                for (const peer of peers) {
                    ...
                    for (const pair of pairedTracks.values()) {
                        ...
                        session.start({
                            ...
                            const audioDescription = pair.audio?.name
                                ? {
                                    application: {
                                ...
                                : undefined;
                            const videoDescription = pair.video?.name || pair.video?.screenCapture
                                ? {
                                    application: {
                                        applicationType: stanza_1.Namespaces.NS_JINGLE_RTP_1,
