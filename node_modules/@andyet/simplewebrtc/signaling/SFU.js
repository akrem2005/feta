...
class SFU extends events_1.EventEmitter {
    constructor(client, browser) {
        ...
        const config = (0, Selectors_1.getAPIConfig)(this.getState()).sfuServer;
        ...
        if (count < this.minimumPeerCount && this.enabled && !this.shutdownTimeout) {
            ...
            this.shutdownTimeout = setTimeout(async () => {
                ...
                if (this.peerCount < this.minimumPeerCount) {
                    ...
                    this.sessions = new Set();
                    await Promise.all([...this.sharedMedia].map(([id, shared]) => {
                        return this.client
                            .send('DISPOSE', {
                            id: shared.sfuId
                        })
    ...
    async sendMedia(track, stream, screenCapture) {
        ...
        pc.oniceconnectionstatechange = () => {
            if (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') {
                ...
                const connections = (0, Selectors_1.getConnections)(this.getState());
    ...
    createPeerConnection(session, options) {
        if (this.enabled && session.isInitiator) {
            ...
            const peer = (0, Selectors_1.getPeerByAddress)(state, session.peerID);
            const room = (0, Selectors_1.getRoomByAddress)(state, Stanza.JID.toBare(session.peerID));
    ...
    getInfoForPeer(peerAddress) {
        return (0, Selectors_1.getPeerByAddress)(this.getState(), peerAddress);
    }
