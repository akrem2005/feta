...
import { Namespaces, JXT, Utils, JID, createClient, RTT, Constants } from 'stanza';
...
class Mesh {
    constructor(client) {
        ...
        this.sfu = client.sfu;
        this.sfu.on('disabled', () => this.updateConnections('sfu-disabled'));
        this.updateQueue = priorityQueue((time, done) => __awaiter(this, void 0, void 0, function* () {
            ...
            // video tracks. It is specified in bits per second.
            const totalMaxBitrate = (_a = appliedTier[1].totalOutgoingBitrate) !== null && _a !== void 0 ? _a : 1800000;
            const perVideoBitrate = totalMaxBitrate / Math.min((this.sfu.enabled ? 1 : videoPeersCount) * sharedVideoCount, 1);
            const sessionMaxBitrate = Math.min((_b = appliedTier[1].bitrate) !== null && _b !== void 0 ? _b : perVideoBitrate, perVideoBitrate);
            ...
            for (const call of calls) {
                const room = getRoomByAddress(state, call.roomAddress);
                const user = getUserDataForRoom(state, call.roomAddress);
                const peers = getPeersForCall(state, call.roomAddress);
                for (const peer of peers) {
                    const allowConnection = this.connectionFilter(user, peer, 'outgoing');
                    ...
                    for (const conn of connections) {
                        ...
                        let stillSharingAudio = false;
                        let stillSharingVideo = false;
                        if (conn.sendingAudioMediaId) {
                            ...
                            if (wantsAudio &&
                                ...
                                this.sharingFilter({ room, peer, user, media: media[conn.sendingAudioMediaId] })) {
                                stillSharingAudio = true;
                            }
                            else {
                                overSharedSessions.add(conn.id);
                            }
                        ...
                        if (conn.sendingVideoMediaId) {
                            ...
                            if ((wantsVideo || video.screenCapture) &&
                                video &&
                                video.shared &&
                                this.sharingFilter({ room, peer, user, media: video })) {
                                stillSharingVideo = true;
                            }
                            else {
                                overSharedSessions.add(conn.id);
                            }
                        }
                        if (overSharedSessions.has(conn.id)) {
                            if (stillSharingAudio) {
                                needsAudio.add(conn.sendingAudioMediaId);
                            }
                            if (stillSharingVideo) {
                    ...
                    for (const track of sharedMedia) {
                        if (!peerSharedMedia.has(track.id)) {
                            const sharingAllowed = this.sharingFilter({ room, peer, user, media: track });
                            if (sharingAllowed && track.kind === 'audio' && wantsAudio) {
                                needsAudio.add(track.id);
                            }
                            if (sharingAllowed && track.kind === 'video' && wantsVideo) {
                                needsVideo.add(track.id);
                            }
                            if (sharingAllowed && track.kind === 'video' && track.screenCapture && wantsAudio) {
                                needsVideo.add(track.id);
                            }
                        }
                    }
                    ...
                    for (const pair of pairedTracks.values()) {
                                    ...
                                    if (this.sfu.hasSession(session.sid)) {
                                        ...
                                        if (profile === 'off') {
                                            ...
                                            if (session._disabledVideoTrack) {
                                                ...
                                                for (const sender of senders) {
                                                    if (!sender.track) {
                                                        sender.replaceTrack(session._disabledVideoTrack);
                                                        session._disabledVideoTrack = undefined;
                                                        continue;
                                                    }
                                                }
                                            }
                        ...
                        session.start({
                            ...
                            const audioDescription = ((_a = pair.audio) === null || _a === void 0 ? void 0 : _a.name)
                                ? {
                                    application: {
                                ...
                                : undefined;
                            const videoDescription = ((_b = pair.video) === null || _b === void 0 ? void 0 : _b.name) || ((_c = pair.video) === null || _c === void 0 ? void 0 : _c.screenCapture)
                                ? {
                                    application: {
                                        applicationType: Namespaces.NS_JINGLE_RTP_1,
                                        screenCaptures: pair.video.screenCapture ? [{ id: pair.video.id }] : [],
                                        trackData: pair.video.name
                                            ? {
                                ...
                                : undefined;
                            const contents = [audioDescription, videoDescription].filter(x => x !== undefined);
                            if (contents.length) {
    ...
    connectionFilter(user, peer, direction) {
        return true;
    }
    sharingFilter(opts) {
        return true;
...
const defs = [
    ...
    },
    {
        aliases: [
            {
                path: 'iq.jingle.contents.application.trackData',
                selector: Namespaces.NS_JINGLE_RTP_1
            }
        ],
        element: 'data',
...
class SFU extends EventEmitter {
    ...
    hasSession(sid) {
        return this.sessions.has(sid) && this.jingle.sessions[sid].state !== 'ended';
    }
    setPeerCount(count) {
        ...
        if (count >= this.minimumPeerCount && !this.enabled && !this.disposing) {
            this.enabled = true;
            clearTimeout(this.shutdownTimeout);
            this.shutdownTimeout = undefined;
            return true;
        }
        if (count < this.minimumPeerCount && this.enabled && !this.shutdownTimeout) {
            // Allow some leeway in case a peer dropped and is rejoining.
            this.shutdownTimeout = setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                this.shutdownTimeout = undefined;
                ...
                if (this.peerCount < this.minimumPeerCount) {
                    ...
                    this.sessions = new Set();
                    yield Promise.all([...this.sharedMedia].map(([id, shared]) => {
                        return this.client
                            .send('DISPOSE', {
                            id: shared.sfuId
                        })
    ...
    sendMedia(track, stream, screenCapture) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.enabled || this.disposing || this.sharedMedia.has(stream.id)) {
                return;
            }
            const sfuId = Utils.uuid();
            const pc = new RTCPeerConnection({
                iceServers: this.jingle.iceServers
            });
            this.sharedMedia.set(stream.id, { sfuId, pc, ready: false });
            pc.oniceconnectionstatechange = () => {
                if (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') {
                    this.sharedMedia.set(stream.id, { sfuId, pc, ready: true });
    ...
    requestProfile(trackId, profile) {
        ...
        for (const [sessionId, conn] of Object.entries(connections)) {
            const session = this.jingle.sessions[sessionId];
            if (conn.receivingVideoMediaId === trackId && session.state !== 'ended') {
                return session.send('description-info', {
                    contents: [
                        {
                            application: {
                                applicationType: Namespaces.NS_JINGLE_RTP_1,
    ...
    createOffer() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const offer = yield this.client.send('CREATE_CONSUMER', {
                ...
                roomId: this.roomId,
                source: this.sfu.sharedMedia.get(this.sourceId).sfuId,
...
function sleep(timeout, throwError = false) {
    return __awaiter(this, void 0, void 0, function* () {
        return new Promise((resolve, reject) => {
            setTimeout(() => (throwError ? reject() : resolve(undefined)), timeout);
        });
    });
}
/**
 ...
 * @param replaces
 */
function addLocalAudio(track, stream, replaces, monitorVolume = true, name) {
    if (track.kind !== 'audio') {
 ...
 * @param name
 */
function addLocalVideo(track, stream, mirror = true, replaces, name) {
    if (track.kind !== 'video') {
 ...
 * @param name
 */
function addLocalScreen(track, stream, replaces, name) {
    if (track.kind !== 'video') {
        throw new Error('Incorrect media type. Expected video, got: ' + track.kind);
    }
    return (dispatch, getState) => {
 ...
 * @param name string
 */
function addRemoteMedia(roomAddress, peerAddress, id, track, stream, screen, name) {
    return (dispatch, getState) => {
        const state = getState();
 ...
 *
 * @param id string
 * @param profile 'high' | 'medium' | 'low' | 'off'
 */
...
class Audio extends Component {
    ...
    render() {
        return (createElement("audio", Object.assign({ id: this.props.id, className: this.props.className, ref: (el) => {
                this.audio = el;
            } }, { playsInline: true })));
    }
}
...
class PeerList extends Component {
    render() {
        ...
        return render ? render(renderProps) : createElement(Fragment, null, this.props.children);
    }
}
...
class ChatComposers extends Component {
    render() {
        ...
        if (render) {
            ...
            return (createElement(PeerList$1, { room: this.props.room, chatState: "composing" },
...
class LocalMediaList extends Component {
    componentDidUpdate(prevProps) {
        if (!this.props.onMediaAdded && !this.props.onMediaRemoved) {
            return;
        }
        const media = new Map(this.props.media.map(m => [m.id, m]));
        const prevMedia = new Map(prevProps.media.map(m => [m.id, m]));
        const ids = new Set([...media.keys(), ...prevMedia.keys()]);
        for (const id of ids) {
            if (media.has(id) && !prevMedia.has(id) && this.props.onMediaAdded) {
                this.props.onMediaAdded(media.get(id));
            }
            if (!media.has(id) && prevMedia.has(id) && this.props.onMediaRemoved) {
                this.props.onMediaRemoved(prevMedia.get(id));
            }
        }
    }
    render() {
...
class Notifications extends Component {
    render() {
        ...
        return render ? render() : createElement(Fragment, null, this.props.children);
    }
    componentDidUpdate(prevProps) {
...
class Provider extends Component {
    ...
    componentDidUpdate(prevProps) {
        if (this.props.signalingClient && this.props.signalingClient !== prevProps.signalingClient) {
            if (this.props.sharingFilter) {
                this.props.signalingClient.mesh.sharingFilter = this.props.sharingFilter;
            }
            else {
                this.props.signalingClient.mesh.sharingFilter = () => true;
            }
        }
    }
    componentWillUnmount() {
 ...
 * </NotSupported>
 */
const NotSupported = connect$1(mapStateToProps$9)(class extends Component {
    render() {
 ...
 * </NotConnected>
 */
const NotConnected = connect$1(mapStateToProps$9)(class extends Component {
    render() {
        const renderProps = {
            connectionState: this.props.connectionState
        };
...
function mapStateToProps$a(state) {
    ...
    for (const audio of media) {
        ...
        audioSources.push({
            media: audio,
            volumeLimit: (_a = (peer ? peer.volumeLimit : 1)) !== null && _a !== void 0 ? _a : 1
        });
    }
    return {
        audioSources,
    ...
    getDisplayMedia() {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                this.props.addLocalScreen(video, stream, this.props.replaceVideo, (_a = this.props.videoName) !== null && _a !== void 0 ? _a : this.props.name);
                ...
                if (audio) {
                    ...
                    this.props.addLocalAudio(audio, stream, this.props.replaceAudio, volumeMonitoring, (_b = this.props.audioName) !== null && _b !== void 0 ? _b : this.props.name);
                    if (this.props.share !== false) {
...
function mapDispatchToProps$7(dispatch) {
    return {
        addLocalAudio: (track, stream, replaceAudio, volumeMonitoring, name) => dispatch(addLocalAudio(track, stream, replaceAudio, volumeMonitoring, name)),
        addLocalScreen: (track, stream, replaceVideo, name) => dispatch(addLocalScreen(track, stream, replaceVideo, name)),
        shareLocalMedia: (id) => dispatch(shareLocalMedia(id))
    };
}
    ...
    getMedia(additional = {}) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (audio) {
                ...
                if (audio['contentHint'] && this.props.audioTypeHint) {
                    audio.contentHint = this.props.audioTypeHint;
                }
                let volumeMonitoring = true;
                if (this.props.volumeMonitoring !== undefined) {
                ...
                this.props.addLocalAudio(audio, stream, this.props.replaceAudio, volumeMonitoring, (_a = this.props.audioName) !== null && _a !== void 0 ? _a : this.props.name);
                if (this.props.share !== false) {
                    this.props.shareLocalMedia(audio.id);
                }
            }
            ...
            if (video) {
                if ('contentHint' in video) {
                    video.contentHint = (_b = this.props.videoTypeHint) !== null && _b !== void 0 ? _b : '';
                }
                if (this.props.screenCapture) {
                    this.props.addLocalScreen(video, stream, this.props.replaceVideo, (_c = this.props.videoName) !== null && _c !== void 0 ? _c : this.props.name);
                }
                else {
                    this.props.addLocalVideo(video, stream, this.props.mirrored, this.props.replaceVideo, (_d = this.props.videoName) !== null && _d !== void 0 ? _d : this.props.name);
                }
                if (this.props.share !== false) {
                    this.props.shareLocalMedia(video.id);
                }
...
function mapDispatchToProps$8(dispatch) {
    return {
        addLocalAudio: (track, stream, replace, volumeMonitoring, name) => dispatch(addLocalAudio(track, stream, replace, volumeMonitoring, name)),
        addLocalScreen: (track, stream, replace, name) => dispatch(addLocalScreen(track, stream, replace, name)),
        addLocalVideo: (track, stream, mirrored, replace, name) => dispatch(addLocalVideo(track, stream, mirrored, replace, name)),
...
class Room extends Component {
    ...
    componentDidUpdate(prevProps) {
        if (this.props.name !== prevProps.name && prevProps.room) {
            if (prevProps.name && prevProps.room) {
                this.props.leave(prevProps.room.address, false);
                this.props.join();
                return;
            }
        }
...
function mapDispatchToProps$9(dispatch, props) {
    return {
        ...
        join: () => dispatch(joinRoom(props.name, {
            autoJoinCall: props.autoJoinCall,
            password: props.password || undefined
        })),
        leave: (roomAddress, endMedia) => dispatch(leaveRoom(roomAddress, endMedia)),
...
class Video extends Component {
    ...
    componentWillUnmount() {
        if (this.props.qualityProfile) {
            this.props.requestQualityProfile('off');
        }
    }
    setup(prev) {
    ...
    render() {
        ...
        return (createElement("video", { id: this.props.id, className: this.props.className, ref: (el) => {
                this.video = el;
