...
exports.removeAllMedia = exports.resumeSelfVideo = exports.pauseSelfVideo = exports.unmuteSelf = exports.muteSelf = exports.unmutePeer = exports.mutePeer = exports.requestQualityProfile = exports.setVideoResolutionTiers = exports.adjustVideoCaptureResolution = exports.stopSharingLocalMedia = exports.shareLocalMedia = exports.disableMedia = exports.enableMedia = exports.updateMedia = exports.removeMedia = exports.addRemoteMedia = exports.addLocalScreen = exports.addLocalVideo = exports.addLocalAudio = exports.addLocalMedia = void 0;
...
function addLocalMedia(media) {
    return (dispatch, getState) => {
        ...
        if (media.replaces) {
            ...
            const prevMedia = (0, Selectors_1.getMediaTrack)(state, media.replaces);
            if (prevMedia) {
                if (!prevMedia.shared) {
...
function addLocalAudio(track, stream, replaces, monitorVolume = true, name) {
    ...
    return (dispatch, getState) => {
        ...
        if ((0, Platform_1.isBrowser)()) {
            ...
            (0, MediaLoaders_1.waitForMediaLoaded)(track, stream).then(info => {
...
function addLocalVideo(track, stream, mirror = true, replaces, name) {
    ...
    return (dispatch, getState) => {
        ...
        if (!(0, Platform_1.isBrowser)() && replaces) {
            // React-Native
...
function addRemoteMedia(roomAddress, peerAddress, id, track, stream, screen, name) {
    return (dispatch, getState) => {
        const state = getState();
        const owner = (0, Selectors_1.getPeerByAddress)(state, peerAddress);
        track.onended = () => {
            dispatch(removeMedia(id));
        };
        if ((0, Platform_1.isBrowser)()) {
            (0, MediaLoaders_1.waitForMediaLoaded)(track, stream, 500).then(info => {
...
function removeMedia(id, endMedia = true) {
    return (dispatch, getState) => {
        const media = (0, Selectors_1.getMediaTrack)(getState(), id);
        if (!media) {
            return;
        }
        if (media.shared) {
        ...
        if (media.source === 'local') {
            const client = (0, Selectors_1.getClient)(getState());
            if (client) {
...
function updateMedia(id, updated) {
    return (dispatch, getState) => {
        ...
        const client = (0, Selectors_1.getClient)(prevState);
        const wasSpeaking = (0, Selectors_1.userIsSpeaking)(prevState);
        dispatch({
            payload: {
                id,
                updated
        ...
        const nowSpeaking = (0, Selectors_1.userIsSpeaking)(newState);
        if (client) {
        ...
        const oldMedia = (0, Selectors_1.getMediaTrack)(prevState, id);
        const newMedia = (0, Selectors_1.getMediaTrack)(newState, id);
        if (newMedia) {
            ...
            if (newMedia.source === 'remote' && newMedia.owner) {
                const peer = (0, Selectors_1.getPeerByAddress)(newState, newMedia.owner);
                ...
                const allPeerMedia = (0, Selectors_1.getMediaForPeer)(newState, newMedia.owner);
...
function shareLocalMedia(id) {
    return (dispatch, getState) => {
        const state = getState();
        const media = (0, Selectors_1.getMediaTrack)(state, id);
        if (!media) {
            return;
        }
        if (media.replaces) {
...
function stopSharingLocalMedia(id) {
    return (dispatch, getState) => {
        const state = getState();
        const potentialReplacements = (0, Selectors_1.getLocalMedia)(state).filter(m => m.replaces === id);
        dispatch(updateMedia(id, {
            shared: false
        }));
...
function adjustVideoCaptureResolution(width, height, frameRate = 30) {
    return (dispatch, getState) => {
        ...
        const state = getState();
        const localMedia = (0, Selectors_1.getLocalMedia)(state, 'video');
...
function mutePeer(peerAddress) {
    return (dispatch, getState) => {
        const state = getState();
        const media = (0, Selectors_1.getMediaForPeer)(state, peerAddress, 'audio');
...
function muteSelf() {
    return (dispatch, getState) => {
        const state = getState();
        const media = (0, Selectors_1.getLocalMedia)(state, 'audio');
        for (const audio of media) {
            dispatch(disableMedia(audio.id));
        }
    };
...
function pauseSelfVideo(opts = { screenCapture: true }) {
    return (dispatch, getState) => {
        const state = getState();
        const media = (0, Selectors_1.getLocalMedia)(state, 'video');
...
function removeAllMedia(kind) {
    return (dispatch, getState) => {
        const state = getState();
        const localMedia = (0, Selectors_1.getLocalMedia)(state, kind);
