...
exports.roomReplaced = exports.roomUnlocked = exports.roomLocked = exports.destroyRoom = exports.unlockRoom = exports.lockRoom = exports.leaveRoom = exports.selfUpdated = exports.joinRoomSuccess = exports.joinRoomFailed = exports.joinRoom = exports.fetchRoomConfig = void 0;
...
function joinRoom(roomName, opts = {}) {
    return async (dispatch, getState) => {
        ...
        const client = (0, Selectors_1.getClient)(state);
        const apiConfig = (0, Selectors_1.getAPIConfig)(state);
        try {
            const config = await fetchRoomConfig(apiConfig.roomConfigUrl, roomName, apiConfig.credential);
            const existingRoom = (0, Selectors_1.getRoomByAddress)(state, config.roomAddress);
            if (!existingRoom || (existingRoom && !existingRoom.joined)) {
                dispatch({
                    payload: {
                        autoJoinCall: (0, Selectors_1.isSupportedBrowser)(state) &&
...
function leaveRoom(roomAddress, endMedia = true) {
    return (dispatch, getState) => {
        const state = getState();
        const client = (0, Selectors_1.getClient)(state);
        dispatch((0, Calls_1.leaveCall)(roomAddress, endMedia));
        if (client) {
...
function roomReplaced(oldRoomAddress, newRoomAddress, newRoomPassword) {
    return async (dispatch, getState) => {
        const state = getState();
        const client = (0, Selectors_1.getClient)(state);
        const oldRoom = (0, Selectors_1.getRoomByAddress)(state, oldRoomAddress);
        if (!oldRoom) {
        ...
        const existingNewRoom = (0, Selectors_1.getRoomByAddress)(state, newRoomAddress);
